  ...
  return <div className="landing">
    ...
    <main id="main">
      <section className="landing-hero">
        <div><span className="eyebrow">YOUR DAILY STUDY SPACE</span><h1>Learn Everyday.</h1><p>Turn your study material into flashcards, self-tests and quick quizzes.</p><Link href="/dashboard" className="button primary">Get Started <ArrowRight size={18}/></Link><small>No account needed. Notes stay on your device.</small></div>
      ...
      <section id="features" className="landing-section"><span className="eyebrow">YOUR STUDY TOOLKIT</span><h2>Less friction. More learning.</h2><div className="landing-tools">{tools.map(t=><Link href={t.href} className="mode-card" key={t.href}><FeatureFigure kind={t.figure}/><h3>{t.name}</h3><p>{t.text}</p><ArrowRight size={20}/></Link>)}</div></section>
      <section id="about" className="landing-about"><div><span className="eyebrow">AT YOUR OWN PACE</span><h2>A clearer next step.</h2></div><p>Your notes. Your rhythm. One focused session at a time.</p></section>
